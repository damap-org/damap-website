...
|-----------------------------|--------------------------------------------------------------------------------------------|
| `DAMAP_DATASOURCE_URL`      | The JDBC connect string in the format of `jdbc:postgresql://SERVERNAME:5432/DATABASENAME`. |
...
| `DAMAP_DATASOURCE_DIALECT`  | Dialect for the Hibernate ORM. Defaults to `org.hibernate.dialect.PostgreSQLDialect`.      |
| `DAMAP_DATASOURCE_DRIVER`   | Driver for your database. Defaults to `org.postgresql.Driver`.                             |
