...
To set up Pure as a project database, set the `DAMAP_PROJECTS_SERVICE` to `elsevier-pure`.

## Setting up as a persons database

To set up Pure as a persons database, set the following environment variables:

- `DAMAP_PERSON_SERVICE_DISPLAY_TEXT` to the text which gets displayed in the frontend when picking the service type, defaults to `University`
- `DAMAP_PERSON_SERVICE_QUERY_VALUE` to `PURE`
- `DAMAP_PERSON_SERVICE_CLASS_NAME` to `org.damap.base.integration.pure.PurePersonService`